...
def filter_runs_to_should_retry(
    ...
    def get_retry_number(run: DagsterRun) -> Optional[int]:
        ...
        # NOTE: group these to reduce db calls
...
def retry_run(
    ...
    if not failed_run.external_pipeline_origin:
        ...
        logger.error(
            "Run does not have an external pipeline origin, this shouldn't be possible unless users are creating runs manually?",
...
def consume_new_runs_for_automatic_reexecution(
    ...
    for run, retry_number in filter_runs_to_should_retry(
        ...
        try:
            ...
            # NOTE: we we likely don't want to expose the whole exception here, and should catch specific cases instead
            ...
            logger.exception(
                "Hit an exception while retrying a run that wasn't explicity handled. "
                "We should catch this explicitly to surface a better user error message.",
                run_id=run.run_id,
            )
