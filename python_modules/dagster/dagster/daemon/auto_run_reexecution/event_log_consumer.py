...
class EventLogConsumerDaemon(IntervalDaemon):
    ...
    def run_iteration(self, instance: DagsterInstance, workspace: IWorkspace):
        ...
        events = []
        new_cursors = {}  # keep these in memory until we handle the events
            ...
            for fn in self.handle_updated_runs_fns:
                try:
                    ...
                    self._logger.exception(
                        "Error calling handler fn: {handler_fn}".format(
...
def _fetch_persisted_cursors(
    ...
    for event_type in event_types:
        ...
        if raw_cursor_value is None:
            logger.warn("No cursor for event type {}, starting from 0!".format(event_type))
            raw_cursor_value = "0"
        ...
        try:
            ...
            logger.exception(
                "Invalid cursor for event_type {}: {}. Crashing!".format(
                    event_type, raw_cursor_value
                )
            )
            raise
