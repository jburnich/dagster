    ...
    def execute(self):
        ...
        with DagsterInstance.from_ref(self.instance_ref) as instance:
            ...
            log_manager = create_context_free_log_manager(
                instance, self.pipeline_run, pipeline.get_definition()
            )
...
def execute_step_out_of_process(
    ...
    yield DagsterEvent.step_process_starting(
        step_context,
        message="Launching subprocess for {}".format(step.key),
        metadata_entries=[MetadataEntry("step_key", value=step.key)],
